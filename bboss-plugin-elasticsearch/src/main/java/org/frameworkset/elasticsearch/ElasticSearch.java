...
import static org.frameworkset.elasticsearch.ElasticSearchSinkConstants.DEFAULT_SERIALIZER_CLASS;
import static org.frameworkset.elasticsearch.ElasticSearchSinkConstants.DEFAULT_TTL;
import static org.frameworkset.elasticsearch.ElasticSearchSinkConstants.HOSTNAMES;
...
//		            ex);
  ...
  public void configure() {
      ...
      if (SimpleStringUtil.isNotEmpty(elasticsearchPropes.getProperty(HOSTNAMES))) {
        serverAddresses = 
        		elasticsearchPropes.getProperty(HOSTNAMES).trim().split(",");
      }
...
//      indexnameBuilderContext.put(INDEX_NAME, indexName);
  ...
  public void start() {
    ...
    try {
        ...
        client = clientFactory.getClient(clientType, serverAddresses,this.elasticUser,this.elasticPassword,
            clusterName, eventSerializer, indexRequestFactory,extendElasticsearchPropes);
        client.configure(elasticsearchPropes);
